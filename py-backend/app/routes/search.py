...
# GET /search?type=business for new code — it adds image/pagination.
@router.get("/search/companies")
async def search_companies(q: str = ""):
    if not q.strip():
        return []
    try:
        rows = await asyncio.to_thread(b2b_search.search_companies_sync, q)
        return [_shape_company(r) for r in rows]
    except Exception as err:
...
# Kept for backward compatibility — prefer GET /search?type=product.
...
async def search_products(q: str = ""):
    if not q.strip():
        return []
    try:
        b2b_rows = await asyncio.to_thread(b2b_product_search.search_products_sync, q)
        local_rows = await asyncio.to_thread(search_local_products_sync, q)
        all_rows = b2b_rows + local_rows
        return [_shape_product(r) for r in all_rows]
    except Exception as err:

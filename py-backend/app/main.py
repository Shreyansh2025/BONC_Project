...
async def lifespan(app: FastAPI):
    ...
    async def _connect():
        ...
        #
        # Each build runs in its OWN try/except. Previously these four
        # awaits shared one try/except, so an exception in an earlier
        # build (e.g. b2b_product_search) silently skipped every build
        # after it (e.g. product_search) — both product indexes would end
        # up empty while company search kept working, with only a single
        # generic error line to explain why.
        try:
